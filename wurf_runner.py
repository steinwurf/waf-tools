...
def make_run(taskgen, run_type):
    ...
    if hasattr(taskgen, 'link_task'):
        ...
        # Locate the shared lib(s) this binary is depending on (if any).
        ...
        for lib_name in taskgen.uselib:
            ...
            if lib_task_gen and lib_task_gen.target:
                # Some lib found, now detect whether it's a shared lib.
                lib_node = lib_task_gen.link_task.outputs[0]

                # To detect whether this lib is a shared lib, we use the
                # cshlib_PATTERN to construct its shared-lib-name. If that
                # matches the name of the lib the lib us assumed to be a shared
                # lib
                shared_lib_name = \
                    taskgen.env['cshlib_PATTERN'] % lib_task_gen.target

                if lib_node.name == shared_lib_name:
                    # It's a shared lib - add it to the test inputs.
